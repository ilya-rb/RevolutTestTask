...
class ResultCallAdapterFactory @Inject constructor(
    private val errorCreator: ApiErrorMapper
) : CallAdapter.Factory() {
...
class ResultAdapter(
    ...
    override fun adapt(call: Call<Type>): Call<Result<Type>> = ResultCall(call, errorCreator)
}
    ...
    override fun enqueueImpl(callback: Callback<Result<T>>) = proxy.enqueue(object : Callback<T> {
        ...
        override fun onResponse(call: Call<T>, response: Response<T>) {
            val result: Result<T> = if (response.isSuccessful) {
                ...
                Result.Err(errorCreator.mapFromErrorBody(response.errorBody()))
            }
        ...
        override fun onFailure(call: Call<T>, t: Throwable) {
            callback.onResponse(
                this@ResultCall,
                Response.success(Result.Err(errorCreator.mapFromThrowable(t)))
            )
        }
    })
}